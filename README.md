...
동기 및 진행경과 : 
- MoA 공연/전시 게시판 프로젝트에서 게시판에 넣어줄 데이터를 mock데이터를 쓸지, 실제 전시 데이터를 쓸지를 선택하는 과정에서, 각 기관들의 데이터들이 통합되어 있다고 하여 ready-to-use의 clean데이터일 것으로 예상하고 공공데이터를 수집해 쓰려고 했는데 실제 데이터는 그렇지 않았고, 10년이상 누적된, 다기관 데이터여서 데이터의 variation이 심했으며, 전반적으로 DB에 바로 삽입하여 사용할 수 있는 정도로 규격화가 되어있지 않았다. 이런 상황이 초기 ERD(개체-관계 모델) 설계 시 데이터의 통계적 특성(예: 필드의 분포, 빈도, 데이터 길이 등)반영이 충분히 고려되지 않았을 때 DB 삽입(Insert)하는데 있어서 어려움을 초래하는 원인이 되었다. <br>
